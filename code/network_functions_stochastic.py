...
def set_index_case(
    ...
    index_farm_idx = choices(farm_indices, k=1)[0]  # returns a list so need the first element
            ...
            if len(indirect_contacts) > 0:
                ...
                for curr_tour in indirect_contacts:
                    ...
                    if curr_tour[CNTCT] == 'i':
                        ...
                        pig_inf_pigs = np.random.poisson(TAU * (tran_inf_pigs / inf_farm_tour[T_NPIGS]) * PIG_PIG_TRANS_RATE *
                                                         pigs_2_dest)
